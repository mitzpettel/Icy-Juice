...
namespace ICQ2000
{
  ...
  void Client::Init()
  {
    ...
    m_transmission_times_index = 0;
    
    for (int i = 0; i<80; i++)
    {
        m_transmission_times[i].tv_sec = 0;
        m_transmission_times[i].tv_usec = 0;
    }
    ...
    m_message_handler->filetransfer_want_file_signal.connect( filetransfer_want_file_signal );
  }
  ...
  void Client::SignalTypingIndicator(TypingIndicatorSNAC *snac)
  {
    if ( m_contact_tree.exists( snac->getUIN() ) )
      m_contact_tree[ snac->getUIN() ]->setTypingFlags( snac->getFlags() );
  }
  ...
  void Client::mergeSBL(ContactTree& tree)
  {
    ...
    while (curr != tree.end())
    {
      ...
      if ( !m_contact_tree.exists_group( (*curr).get_id() ) )
      {
         m_contact_tree.add_group( (*curr).get_label(), (*curr).get_id() );
      }
      
      ContactTree::Group *local_group = &(m_contact_tree.lookup_group( (*curr).get_id() ));

      std::cout << "Group: " << (*curr).get_label() << endl;
      while (gcurr != (*curr).end())
      {
	std::cout << "  Contact: " << (*gcurr)->getAlias() << endl;
        if ( !m_contact_tree.exists( (*gcurr)->getUIN() ) )
        {
            local_group->add( *gcurr );
        }
	++gcurr;
      }
      ++curr;
  ...
  void Client::SignalRateInfoChange(RateInfoChangeSNAC *snac)
  {
    RateInfoChangeEvent e(snac->getCode(), snac->getRateClass(),
			  ...
			  snac->getMaxAvg());
ostringstream ostr;
ostr << "Rate info: code:" << snac->getCode()
     << " rateClass:" << snac->getRateClass()
     << " window size:" << snac->getWindowSize()
     << " clear:" << snac->getClear()
     << " alert:" << snac->getAlert()
     << " limit:" << snac->getLimit()
     << " disconnect:" << snac->getDisconnect()
     << " currentAvg:" << snac->getCurrentAvg()
     << " maxAvg:" << snac->getMaxAvg() ;
SignalLog( LogEvent::INFO, ostr.str() );

    if (snac->getCurrentAvg() <= snac->getLimit())
    {
    }
  ...
  void Client::SendAuthReq() {
    ...
    b << ScreenNameTLV(m_self->getStringUIN())
      ...
      << ClientProfileTLV("ICQ Inc. - Product of ICQ (TM).2002a.5.37.1.3728.85"/*"ICQ Inc. - Product of ICQ (TM).2000b.4.63.1.3279.85"*/)
      << ClientTypeTLV(266)
      << ClientVersionMajorTLV(5/*4*/)
      << ClientVersionMinorTLV(37/*63*/)
      << ClientICQNumberTLV(1)
      << ClientBuildMajorTLV(3828/*3279*/)
  ...
  void Client::SendEnqueued() {

    struct timeval now;
    gettimeofday(&now, NULL);
    while (!m_send_queue.empty() && ( now.tv_sec - m_transmission_times[m_transmission_times_index].tv_sec > 30
         || (now.tv_sec - m_transmission_times[m_transmission_times_index].tv_sec)*1000000 +(now.tv_usec - m_transmission_times[m_transmission_times_index].tv_usec) > 25000000))
    {
        try {
        ostringstream ostr;
        ostr << "Sending packet to Server" << endl << m_send_queue.front();
        SignalLog(LogEvent::PACKET, ostr.str());
        m_serverSocket->Send(m_send_queue.front());
        m_transmission_times[m_transmission_times_index++] = now;
        m_transmission_times_index %= 40;
        m_send_queue.pop();
        } catch(SocketException e) {
        ostringstream ostr;
    ...
    {
        ostringstream ostr;
        ostr << "Send queue remains at: " << m_send_queue.size();
        SignalLog(LogEvent::INFO, ostr.str());
    }
  }    
  ...
  void Client::ParseCh2(Buffer& b, unsigned short seq_num)
  {
	...
	break;
      case SNAC_MSG_TypingIndicator:
        SignalLog(LogEvent::INFO, "Received MTN from server");
        SignalTypingIndicator(static_cast<TypingIndicatorSNAC*>(snac));
        break;
      }
      break;
  ...
  bool Client::SendDirect(MessageEvent *ev) {
    ...
    if (ev->getType()==MessageEvent::Normal)
        if (dynamic_cast<NormalMessageEvent *>(ev)->isMultiParty() || dynamic_cast<NormalMessageEvent *>(ev)->getTextEncoding()==ENCODING_UCS2) return false;
  ...
  void Client::SendViaServer(MessageEvent *ev)
  {
	...
	|| ev->getType() == MessageEvent::Contacts)
    {
      ...
      if (c->get_accept_adv_msgs()
            && !(ev->getType()==MessageEvent::Normal && ( dynamic_cast<NormalMessageEvent *>(ev)->isMultiParty() || dynamic_cast<NormalMessageEvent *>(ev)->getTextEncoding()==ENCODING_UCS2 )))
...
//    FLAPwrapSNAC( b, SrvUpdateWorkInfo(m_self->getUIN(), ow) );
  ...
  void Client::fetchDetailContactInfo(const ContactList& l) {

    SignalLog(LogEvent::INFO, "Sending request Detailed Userinfo Requests");
    ...
    while (curr != l.end())
    {
        ...
        {
            unsigned int reqid = NextRequestID();
            m_reqidcache->insert( reqid, new UserInfoCacheValue( getContact((*curr)->getUIN()) ) );
  ...
  void Client::fetchBasicInfoForUin(const unsigned int uin) {
    FLAPwrapSNACandSend( SrvRequestSimpleUserInfo( m_self->getUIN(), uin ) );
  }
  ...
  void Client::FileTransferSkipFile(FileTransferEvent *ev)
  {
    ev->getFileTransferClient()->SkipFile();
  }
  ...
  void Client::SendFileTransfer(FileTransferEvent *ev)
  {
    ...
    if ( ev->getState() == FileTransferEvent::RECEIVE )
    {
      ev->getFileTransferClient()->ReceiveFile();
      return;
    }
  ...
  void Client::CancelFileTransfer(FileTransferEvent *ev)
  {
	  if (ev != NULL) {
	    ...
	    case FileTransferEvent::TIMEOUT:
            case FileTransferEvent::SEND:
            case FileTransferEvent::RECEIVE:
		    FileTransferClient *ftc = m_ftcache->getByEvent(ev);
		    if ( ftc != NULL && ftc->getSocket() )
			 DisconnectDirectConn(ftc->getfd());
		    break;	
	    }
	  }
