...
namespace ICQ2000
{
  ...
  FileTransferClient::~FileTransferClient()
  {
    ...
    if (m_ev)
      m_ev->setFileTransferClient( NULL );
  }
  ...
  void FileTransferClient::Init()
  {
    ...
    m_socket = NULL;
    m_ev->setFileTransferClient( this );
  }
  ...
  void FileTransferClient::ReceiveFile()
  {
    if ( m_waiting_for_fout )
    {
      m_waiting_for_fout = false;
  ...
  void FileTransferClient::SkipFile()
  {
    m_waiting_for_fout = false;
    SendPacket0x04();
    if ( m_ev->getCurrFile() == m_ev->getTotalFiles() )
    {
...
//      SendPacket0x03(0,m_ev->getCurrFile());
  ...
  void FileTransferClient::ParsePacket0x02(Buffer& b)
  {
    ...
    m_ev->setTotalPos( m_ev->getTotalPos()-m_ev->getPos()+m_ev->getSize() );
    m_ev->setSize(size);
    m_ev->setPos(0);
    m_ev->setCurrFile(m_ev->getCurrFile()+1);
    m_ev->setSpeed(speed);
    m_ev->setCurrFilename( filename );
/*
    m_path = m_ev->getSavePath(); 	
    ...
    m_message_handler->handleUpdateFT(m_ev);
*/
    // Tell the client we need it to set up the stream to receive the file. When it's set up, the client should call us at ReceiveFile (currently done by calling Client::SendFileEvent() ! ).
    m_waiting_for_fout = true;
    m_message_handler->handleWantFile( m_ev, &m_fout );
  }
  ...
  void FileTransferClient::ParsePacket0x03(Buffer& b)
  {
    ...
    if ( nfiles > m_ev->getTotalFiles() )
    {
        // this is how Mac ICQ asks to skip a file before it even starts (nfiles==0x1021120)
        m_more = false;
        m_continue = true;
        if ( m_ev->getCurrFile() == m_ev->getTotalFiles() )
        {
  ...
  void FileTransferClient::ParsePacket0x04(Buffer& b)
  {
    unsigned int filenum;
    
    b >> filenum;
    if ( filenum == m_ev->getCurrFile() )
    {
        m_more = false;
        m_continue = true;
        if ( filenum == m_ev->getTotalFiles() )
        {
            m_ev->setState(FileTransferEvent::COMPLETE);
            throw DisconnectedException("FileTransfer is Complete");    
        }
  ...
  void FileTransferClient::SendPacket0x02()
  {
    ...
    m_message_handler->handleWantFile( m_ev, &m_fin );
    
// client should setSize, setCurrFilename, and open the file for us!    
    b.setLittleEndian();
    
    Buffer::marker m1 = b.getAutoSizeShortMarker();
    b << (unsigned char)0x02;
    b << file_or_dir;
    b.PackUint16StringNull( m_ev->getCurrFilename() );  //name of next file.
  ...
  void FileTransferClient::SendPacket0x04()
  {
    Buffer b;
    b.setLittleEndian();
    Buffer::marker m1 = b.getAutoSizeShortMarker();
    b << (unsigned char)0x04;  
    b << m_ev->getCurrFile();
...
// MITZ alternative for readsome for gcc-3.1. actually buggy in 3.3 as well?!s
    m_fin.read((char *)m_const_buf, 2048);
    length = MIN( 2048, m_ev->getSize()-m_ev->getPos() );
...
//    ev->setTotalFiles(ev->getFilesInQueue()); it is the client's responsibility to set up the promised number of files
    ev->setCurrFile(0);
