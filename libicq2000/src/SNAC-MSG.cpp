...
namespace ICQ2000 {
  ...
  void MsgSendSNAC::OutputBody(Buffer& b) const
  {
    ...
    if (m_icqsubtype->getType() == MSG_Type_Normal) {
	  ...
	  Encoding encoding = nst->getTextEncoding();
	  bool sendUCS2 = ( encoding==ENCODING_UCS2 || encoding==ENCODING_UTF8 );
      ...
      // flags - for what?
      b << (unsigned short) ( sendUCS2 ?  MESSAGETEXT_FLAG1_UCS2 : 0x0000 )
	<< (unsigned short)0x0000;
      ...
      if ( encoding==ENCODING_UTF8 )
	  {
		...
		while ( i<text.length() )
		{
		  if ( (unsigned char)text[i]<0x80 )
		    b << (unsigned short) (unsigned char)text[i++];
		  else if ( (unsigned char)text[i]<0xe0 )
		    b << (unsigned short) ( ((unsigned char)text[i++]<<6 & 0x7c0) | ((unsigned char)text[i++] & 0x3f ) );
		  else
		    b << (unsigned short) ( ((unsigned char)text[i++]<<12 & 0xf000 ) | ((unsigned char)text[i++]<<6 & 0xfc0 ) | ((unsigned char)text[i++] & 0x3f ) );
		}
	  }
	  else
	    b.Pack(text);
  ...
  TypingIndicatorSNAC::TypingIndicatorSNAC(unsigned int uin, unsigned short flags)
    : m_uin(uin), m_flags(flags) { }

  void TypingIndicatorSNAC::OutputBody(Buffer& b) const {
  ...
  void TypingIndicatorSNAC::ParseBody(Buffer& b)
  {
    b.advance(10);
    string uinstr;
    b.UnpackByteString( uinstr );
    m_uin = Contact::StringtoUIN( uinstr );
    b >> m_flags;
  }
